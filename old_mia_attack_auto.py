...
def lira_attack_ldh_cosine(f,epch,K, save_dir, extract_fn=None,attack_mode="cos"):
    ...
    if select_mode == 1 and attack_mode =='cos':
        ...
        if select_method == 'outlier':
            ...
            for j in range(0,shadow_train_losses_stack.shape[1]):
                print('outlier filter start......')
                mask1 = shadow_train_losses_stack[:,j] < shadow_train_losses_stack[:,j].mean(axis=0) + 3*shadow_train_losses_stack[:,j].std(axis=0)
                mask2 = shadow_train_losses_stack[:,j] > shadow_train_losses_stack[:,j].mean(axis=0) - 3*shadow_train_losses_stack[:,j].std(axis=0)
            ...
            for j in range(0,shadow_test_losses_stack.shape[1]):
                mask1 = shadow_test_losses_stack[:,j] < shadow_test_losses_stack[:,j].mean(axis=0) + 3*shadow_test_losses_stack[:,j].std(axis=0)
                mask2 = shadow_test_losses_stack[:,j] > shadow_test_losses_stack[:,j].mean(axis=0) - 3*shadow_test_losses_stack[:,j].std(axis=0)
                mask = np.logical_and(mask1, mask2)
    ...
    # if attack_mode == 'cos':
    draw_hist(train_l_out,[0],f'{epch}_{attack_mode}_{MODE}_mem')
    draw_hist([0],test_l_out, f'{epch}_{attack_mode}_{MODE}_nonmem')
...
def cos_attack(f,K,epch,attack_mode,extract_fn=None):
    ...
    if attack_mode =="cosine attack":
        ...
        # draw_hist(train_liratios,[0],f'{epch}_nonlira_{attack_mode}_{MODE}_mem')
        # draw_hist([0],val_liratios, f'{epch}_nonlira_{attack_mode}_{MODE}_nonmem')
        draw_hist(train_liratios,val_liratios,f'{epch}_nonlira_{attack_mode}_{MODE}_all')   
        if epch % 50 ==0:
...
def fig_out(x_axis_data, MAX_K,defence,seed,log_path, d,avg_d=None,single_score=None, other_scores=None,accs=None): 
    ...
    if avg_d:
        for k in avg_d.keys():
            if avg_d[k]:    
                plt.hlines([avg_d[k]["0.001"]],xmin=0,xmax=300,label=k,color=colors[k])
    ...
    pdf_path="/".join(pdf_path)+f"/attack_select_{select_mode}_{select_method}20_{MODE}_n{SHADOW_NUM}_s{SEED}.pdf"
    ...
    log_path="/".join(log_path)+f"/attack_select_{select_mode}_{select_method}20_{MODE}_n{SHADOW_NUM}_s{SEED}.log"
    # print("log_path:",log_path)
    with open(log_path,"w") as f:
        json.dump({"avg_d":avg_d,"single_score":single_score,"other_scores":other_scores,"accs":accs},f, indent=4)
    # assert 0

def draw_hist(scores1, scores2, mode):
    plt.cla()
    # if 'all' in mode:
        # print('max num:')
        # print(scores1.shape)
    ...
    n, bins, patches = plt.hist(scores2, 40, color='blue', density=True, rwidth= 0.8, alpha=0.6,  label='shadow models')
    from scipy.stats import norm
    # 计算正态分布曲线的 y 值
    bin_centers = 0.5 * (bins[1:] + bins[:-1])
    y = n
    # 绘制曲线
    # plt.plot(bin_centers, y, color='red', marker= '.', linewidth=1, label='Distribution of Nonmem')

    n, bins, patches = plt.hist(scores1, 40, color='red', density=True, rwidth= 0.8, alpha=0.6,  label='target model')
    from scipy.stats import norm
    # 计算正态分布曲线的 y 值
    bin_centers = 0.5 * (bins[1:] + bins[:-1])
    y = n
    # 绘制曲线
    # plt.plot(bin_centers, y, color='blue', marker= '.', linewidth=1, label='Distribution of Mem')
    ...
    fig_path=PATH.split("/")[0:-1]
    sav_fig_dir = "/".join(fig_path)+f'/hist_figs/a_sl{select_mode}_{select_method}_s{SEED}_2411_column'
    if os.path.exists(sav_fig_dir) == False:
        os.makedirs(sav_fig_dir)
    fig_path=sav_fig_dir +f"/hist_non_iid_{mode}.pdf"
...
def attack_comparison(p,log_path, save_dir, epochs, MAX_K, defence,seed):
    ...
    draw_hist(train_score,[0],f'0avg_lira_cos_{MODE}_mem')
    draw_hist([0],test_score, f'0avg_lira_cos_{MODE}_nonmem')
    ...
    train_score=np.vstack([ i[0].reshape(1,-1) for i in reses]).mean(axis=0)
    test_score=np.vstack([ i[1].reshape(1,-1) for i in reses]).mean(axis=0)
    draw_hist(train_score,[0],f'0avg_lira_loss_{MODE}_mem')
    draw_hist([0],test_score, f'0avg_lira_loss_{MODE}_nonmem')
    ...
    print(np.mean(train_score),np.var(train_score), train_score.shape)
    print(np.mean(test_score), np.var(test_score), test_score.shape)
    draw_hist(train_score,[0],f'0avg_cos_{MODE}_mem')
    draw_hist([0],test_score, f'0avg_cos_{MODE}_nonmem')
    ...
    print(np.mean(train_score),np.var(train_score), train_score.shape)
    print(np.mean(test_score), np.var(test_score), test_score.shape)
    draw_hist(train_score,[0],f'0avg_loss_{MODE}_mem')
    draw_hist([0],test_score, f'0avg_loss_{MODE}_nonmem')
